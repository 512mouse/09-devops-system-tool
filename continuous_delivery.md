...
——可以定义不同的环境类型，如：开发、测试、预发布及生产环境；

——针对不同的环境类型有不同的集群管理能力，如生产环境有正式和备份不同集群；

——可以定义不同的环境依赖资源信息及其配置，比如主机、容器集群、DNS、中间件、其他基础设施服务等等；

——可以根据环境的配置快速生成可用环境；

——可以集中存储环境的配置信息，并且版本化控制配置信息；

——可以支持应用运行的环境是静态主机集群或者是动态的容器集群；

——可以支持不同的应用有不同的基础设施及服务依赖；

——可以支持不同的对象分块分步构建，比如说构建基础设施、构建中间件或者操作系统环境等等；

——可以支持不同的环境采用不同的构建方法和技术，比如说虚拟化、容器等等，但测试环境和生产环境必须采用相同的构建方法，避免出现环境不一致；
