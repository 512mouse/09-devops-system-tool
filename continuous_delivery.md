...
### 2,构建与持续集成
构建是将软件源代码通过构建工具转换为可执行程序的过程，一般包含编译和链接两个步骤，将高级语言代码转换为可执行的机器代码并进行相应的优化，提升运行效率。持续集成是软件构建过程中的一个最佳实践，在版本控制的基础上，通过频繁的代码提交，自动化构建和自动化测试，加快软件集成周期和问题反馈速度，从而及时验证系统可用性。

#####基本功能：
——构建过程的触发方式支持手动、定时、远程、GIT以及SVN等多种方式；
——构建触发需支持变量方式（变量包括当前构建的启动方式、构建的唯一ID、自定义参数等）；
——构建调度需支持MacOS\Linux\Windows\等主流操操作系统；
——构建环境需支持可定制的运行最长时间（超时时间）；
——构建任务可通过配置模板或脚本定义；
——构建任务支持git\svn等代码仓库的代码拉取；
——构建任务中的git仓库代码拉取需支持按分支、tag、commit ID等方式。
——构建任务中的仓库拉取策略需支持revert、update、fresh、check out、increment update等；
...
——支持测试和代码检查结果的指标作为版本质量的强制要求，并采取质量红线等方式强化代码主干质量；

#####高级功能：
——基于git事件触发的方式，支持监听分支、排除分支的规则，以及支持push\tag\PR事件；
...
——持续集成作为研运一体化的基础服务，为上层SaaS提供可订制的各类组件；
...
——支持对接调度其他服务作为流水线任务，包括但不限于代码托管、代码检查、编译构建、部署、测试、发布等

——支持跳转其他服务查看任务详情

——支持多种触发方式，包括但不限于定时触发、代码提交触发等
...
——支持数据可视化，包括但不限于
  ...
  - 查看单条流水线的实时执行状态、执行结果、执行历史记录
  - 查看多条流水线的执行状态、执行进展、操作
  - 查看单任务执行日志
