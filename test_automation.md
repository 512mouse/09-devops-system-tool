...
——支持测试主流接口形式，包括但不限于REST、RPC风格等

——支持测试HTTP、HTTPS协议的主要方法，包括但不限于GET、POST、PUT、DELETE等

——支持编辑报文，包括但不限于HTTP请求的URL、请求头、请求报文等

——支持多种报文格式，包括但不限于表单、文本、JSON、XML等

——支持接口鉴权认证，包括但不限于Basic认证、Token认证等方式

——支持针对HTTP响应的响应报文、响应头、响应码定义测试断言

——支持提取HTTP响应的内容并基于上下文创建后续HTTP请求
...
——支持编排测试用例执行顺序
  - 可采取测试套件形式进行编排
  - 可设定执行方式，包括但不限于串行、并行（可能不是接口测试专有特性）

——支持调试测试用例（可能不是接口测试专有特性）

——支持记录测试执行日志和结果（可能不是接口测试专有特性）

——支持定义接口Mock

——支持其他服务调用接口测试，包括但不限于流水线服务

——支持统计接口测试覆盖率和通过率

##### 可以包含以下高级功能：

-	支持基于Swagger或Open API文档辅助编写接口测试逻辑
-	支持基于录制的接口请求和响应生成测试逻辑
