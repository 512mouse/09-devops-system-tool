...
专家建议：测试应该执行方来做的，这里所描述的所有测试是执行测试还是包括了管理部分，比如单元测试覆盖率大于70%

##### 应包含以下基本功能：

——自动化、独立性、可重复执行的特点（是单元测试特性）

——每个CASE有assert 来验证

——核心业务、核心应用、核心模块的增量代码确保单元测试通过（不是成熟度，建议去掉）-建议保留

——单元测试的CASE有手工编写和自动化生成2种，文件名要区分

——每个单元测试CASE执行时间不超过1秒可以指定。
...
——单元测试的参数范围包含开发和测试共同确定（建议修改描述，要体现出能力项要求）
